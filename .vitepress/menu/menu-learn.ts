...
const learnRoot = "src/learn";

const learnDir = path.resolve(__dirname, "../..", learnRoot);
const learnBase = `/${learnRoot}`;

/**
 * learn 菜单规则：
 * - src/learn/*.md 生成一级菜单项。
 * - src/learn/<目录>/*.md 生成带 children 的分组菜单。
 */
...
/**
 * 拼接文档路由，避免在多个分支里重复写 /src/learn 前缀。
 */
...
/**
 * 根据 src/learn 的实际产物生成顶部导航和侧边栏。
 */
export default async function menuLearn(): Promise<IMenuRules | undefined> {
  if (!fs.existsSync(learnDir)) {
    return undefined;
  }

  const entries = await fs.promises.readdir(learnDir, {
    withFileTypes: true
  });
  ...
  const menuItems = await Promise.all(entries
    ...
    .map(async entry => {
      const sourcePath = path.join(learnDir, entry.name);
